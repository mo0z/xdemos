...
static inline void hexdump(char *p, size_t l)
{
	const char *fmt = " %02X";
	register size_t i;
	fprintf(stderr, fmt + 1, *p);
	for(i = 1; i < l; i++) {
		fprintf(stderr, fmt, p[i]);
	}
	fputc('\n', stderr);
}
...
int x11_handshake(int sock, struct x11_connection *conn)
{
	...
	// it seems auth_proto/auth_data and the magic cookie are missing here.
	...
	char *p = ((char*)conn->setup)+sizeof(struct x11_conn_setup)+conn->setup->vendor_length;	//Ignore the vendor
	conn->format = (struct x11_pixmap_format*)p;	//Align struct with format sections
	p += sizeof(struct x11_pixmap_format) * conn->setup->formats;	//move pointer to end of section
	conn->root = (struct x11_root_window*)p;	//Align struct with root section(s)
	p += sizeof(struct x11_root_window) * conn->setup->roots;	//move pointer to end of section
	conn->depth = (struct x11_depth*)p; //Align depth struct with first depth section
	p += sizeof(struct x11_depth);	//move pointer to end of section
	conn->visual = (struct x11_visual*)p; //Align visual with first visual for first depth
...
uint32_t x11_generate_id(struct x11_connection *conn)
{
	...
	fprintf(stderr, "derp %lX\n", (uintptr_t)conn->setup);
...
int main(int argc, char **argv)
{
	...
	if(x11_handshake(sockfd,&conn) == 0)
		return EXIT_FAILURE;
